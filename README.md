...
- Engineered an end-to-end Python research pipeline that scores financial news and earnings-call transcripts for the Nifty 50 universe using FinBERT and a Loughran–McDonald finance lexicon, then rigorously tests whether the resulting sentiment predicts forward stock returns.
...
### II. [Multi-Modal Restaurant Recommendation System](https://github.com/CrossAnushka/multimodal-restaurant-recommender)

- Designed an end-to-end hybrid recommender in Python/PyTorch that fuses review text (frozen DistilBERT), food imagery (frozen ResNet18), and structured attributes (cuisine, price, location) into a single learned ranker — combining collaborative and content-based filtering via late fusion, with cold-start handling for both new users and new restaurants.
- Engineered two-level late fusion: per-modality standardize → PCA → L2-norm → concat at the feature level, and a unified MLP that fuses matrix-factorization CF vectors with content and cross-features (cuisine match, price fit, proximity) at the model level; only lightweight layers train, so it runs locally in minutes.
- Validated on held-out ranking metrics — hybrid beats content-only and CF-only on every metric (NDCG@10 0.142 vs 0.099 vs 0.063), ablations confirm each modality adds lift, and cold-start users score NDCG@10 ≈ 0.26. Shipped a FastAPI + single-page frontend serving recs with a per-modality "why" breakdown that makes the fusion contribution visible.

### III. [Bank Customer Churn Prediction (ANN vs. Gradient Boosting)](https://github.com/CrossAnushka/bank-customer-churn-ann)
...
### IV. [Image Classification with Artificial Neural Networks (ANN)](https://github.com/CrossAnushka/Image-Classification-using-ANN)
...
### V. [Breast Cancer Detection using Random Forest](https://github.com/CrossAnushka/breast-cancer-detection)  
...
### VI. [SONAR: Rock vs Mine Prediction](https://github.com/CrossAnushka/SONAR--rock-vs-mine-prediction-model)  
...
### VII. [Handwritten Digit Recognition](https://github.com/CrossAnushka/handwritten-digit-recognition)  
